...
if(!isset($_POST['submit'])){
	...
	*/
	...
	$formClass->only_text($groupware_general->welcome("Ti ricordiamo inoltre che, la continuazione dell'operazione, impone l'accettazione di tutte le regole descritte nel nostro <strong>contratto sociale</strong> presente nella prima parte della pagina. Il suddetto documento <strong>andrà accettato</strong> per poterci consentire la conferma della tua inserzione.<br>Il tuo lavoro sarà pubblicato quindi sotto i termini della licenza <a href=\"#\">GNU GPL</a>.<br>
	<em>Se non riesci a leggere il testo contenuto nell'immagine, ricarica la pagina.</em>"));
	
	# add breakline
	echo("<br>");
	
	# create ID text input
	$formClass->text("__ob_code", "Codice sottostante: ");
	
	# get image
	$formClass->only_text("<img src=\"misc/groupware/image.php?sessid=".session_id()."&type=add_project_confirm_ID\">");
